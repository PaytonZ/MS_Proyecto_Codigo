 ...
 */
public class PanelConsultaMultiplesDepartamentos extends JPanel implements GUIPanelesInterfaz {

	private static final long serialVersionUID = 1L;
	
	private JList<Departamento> list;
	
	private JPanel contentPane;
	
	public PanelConsultaMultiplesDepartamentos() {
		
		contentPane = this;
		
		setLayout(new BorderLayout(0, 0));

		list = new JList<Departamento>();
		...
		JButton btnNewButton = new JButton("Consultar departamentos");
	...
	public void actualizarVentana(IDEventos idEvento, Object datos) {
		...
		if ( IDEventos.EVENTO_CONSULTAR_TODOS_DEPARTAMENTO == idEvento ) {
			...
			if ( datos instanceof List) {
				...
				Departamento[] departamentos = new Departamento[listaDepartamentos.size()];
				listaDepartamentos.toArray(departamentos);
				
				list.setListData(departamentos);
			}
			else {
				JOptionPane.showMessageDialog(contentPane, "No se pudieron recuperar los departamentos", "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
		else if ( IDEventos.ERROR_CONSULTAR_TODOS_DEPARTAMENTO == idEvento ) {
